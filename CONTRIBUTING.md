...
This repo packages [phoenixd](https://github.com/ACINQ/phoenixd) for StartOS.
...
phoenixd runs the `acinq/phoenixd` image pinned via `dockerTag` in `startos/manifest/index.ts`.

1. Bump `dockerTag` in `startos/manifest/index.ts` to `acinq/phoenixd:<new version>`.
